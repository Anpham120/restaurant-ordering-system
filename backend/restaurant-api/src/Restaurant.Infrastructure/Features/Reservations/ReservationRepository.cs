...
namespace Restaurant.Infrastructure.Features.Reservations;
...
public class ReservationRepository(RestaurantDbContext db) : IReservationRepository
{
    ...
    public async Task<List<Domain.Entities.Reservation>> GetReservationsAsync(ReservationStatus? status, DateOnly? date, CancellationToken ct = default)
    {
    ...
    {
        var active = TableSessionStatus.Active.ToString();
        return await db.TableSessions.AnyAsync(s => s.TableId == tableId && s.Status == active, ct);
    }
